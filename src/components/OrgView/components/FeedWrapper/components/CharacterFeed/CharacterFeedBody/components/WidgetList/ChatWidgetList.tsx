...
import { Avatar, Box, Button, Flex, Grid, GridItem, Input, Stack, Text } from "@chakra-ui/react";
                        ...
                        .map((chatWidget) => (
                            <GridItem key={chatWidget.id} w="auto" h="400">
                                <Stack height="100%" justify="space-between" width="100%">
                                    <Stack overflow="hidden" flexGrow={1} justify="space-between" spacing={0}>
                                        <Stack>
                                            <Box height="80px" width="100%" bg={chatWidget.primaryColor} borderTopRadius={10} px={2} py={7}>
                                                <Flex justify="flex-start"  gap={1} align="end">
                                                    <Avatar size="xs" name={character.name} src={character.image} />
                                                    <Text>{character.name}</Text>
                                                </Flex>
                                            </Box>
                                        </Stack>
                                        <Stack bg="white" overflow="hidden" flexGrow={1} justify="flex-end" p={1}>
                                            <Flex justify="flex-start" gap={1} align="end">
                                                <Avatar size="xs" name={character.name} src={character.image} />
                                                <Box height="40px" width="70%" bg="grey" borderRadius={10}></Box>
                                            </Flex>
                                            <Flex justify="flex-end">
                                                <Box height="40px" width="70%" bg={chatWidget.secondaryColor} borderRadius={10}></Box>
                                            </Flex>
                                        </Stack>
                                        <Text
                                            bg="background.700"
